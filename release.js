...
async function loadRelease() {
  ...
  try {
    ...
    document.querySelectorAll(".release-version").forEach((node) => { node.textContent = release.version; });
    document.querySelector("#heroVersion").textContent = release.version;
    ...
    status.innerHTML = `最新版本 <b>${release.version}</b> · 发布于 ${published} · <a href="${release.releaseUrl}">查看 Release 与校验文件 ↗</a>`;
  } catch {
